...
const vehicles = ref<WhitelistVehicleView[]>([])
const total = ref(0)
const page = ref(0)
...
async function loadVehicles(): Promise<void> {
  ...
  const result = await listWhitelistVehicles(selectedLotId.value, locale.value, {
...
async function reload(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = error instanceof ApiError ? error.message : t('whitelist.loadFailed')
  } finally {
    loading.value = false
  }
}
...
function openEditForm(vehicle: WhitelistVehicleView): void {
...
async function onSubmit(): Promise<void> {
  ...
  if (!formPlate.value.trim() || !formOwnerName.value.trim()) {
    formError.value = t('whitelist.formRequired')
    return
  }
  if (!formStartTime.value || !formEndTime.value) {
    formError.value = t('whitelist.formTimeRequired')
    return
  }
  ...
  if (!startTime || !endTime || Date.parse(endTime) <= Date.parse(startTime)) {
    formError.value = t('whitelist.formInvalidTimeRange')
    return
  }
  ...
  try {
    if (isEditing.value && editingId.value) {
      await updateWhitelistVehicle(selectedLotId.value, editingId.value, payload, locale.value)
    } else {
      await createWhitelistVehicle(selectedLotId.value, payload, locale.value)
    }
    closeForm()
    successMessage.value = t('whitelist.saveSuccess')
    await loadVehiclesOnly()
  } catch (error) {
    formError.value = error instanceof ApiError ? error.message : t('whitelist.saveFailed')
  } finally {
    submitting.value = false
  }
}

async function onDelete(vehicle: WhitelistVehicleView): Promise<void> {
  if (!selectedLotId.value) {
    return
  }
  submitting.value = true
  errorMessage.value = ''
  try {
    await deleteWhitelistVehicle(selectedLotId.value, vehicle.id, locale.value)
    successMessage.value = t('whitelist.deleteSuccess')
    await loadVehiclesOnly()
  } catch (error) {
...
async function downloadImportTemplate(): Promise<void> {
  ...
  importError.value = ''
  try {
    await downloadWhitelistImportTemplate(selectedLotId.value, locale.value)
  } catch (error) {
    importError.value = error instanceof ApiError ? error.message : t('whitelist.downloadTemplateFailed')
  } finally {
...
async function onSubmitImport(): Promise<void> {
  if (!selectedLotId.value || !importFile.value) {
    importError.value = t('whitelist.importFileRequired')
    return
  }
  importing.value = true
  importError.value = ''
  try {
    const result = await importWhitelistVehicles(selectedLotId.value, importFile.value, locale.value)
    successMessage.value = t('whitelist.importSuccess', {
    ...
    importError.value = error instanceof ApiError ? error.message : t('whitelist.importFailed')
  } finally {
  ...
  <section class="page">
    ...
    <template v-else>
      ...
      <div class="table-card">
        ...
        <div v-else-if="loading" class="empty">
          <p>{{ t('whitelist.loading') }}</p>
        </div>
        <div v-else class="empty">
          <strong>{{ t('whitelist.empty') }}</strong>
          <p>{{ isAdmin ? t('whitelist.emptyHintAdmin') : t('whitelist.emptyHint') }}</p>
        </div>
    ...
    <div v-if="showForm" class="modal-backdrop">
      <form class="modal" @submit.prevent="onSubmit">
        ...
        <label>
          <span>{{ t('whitelist.colPlateColor') }}</span>
        ...
        <label>
          <span>{{ t('whitelist.colPhone') }}</span>
          <input v-model="formPhone" type="text" autocomplete="off" />
        </label>
        <label>
          <span>{{ t('whitelist.colDepartment') }}</span>
        ...
        <div class="actions">
          <button type="button" class="ghost" @click="closeForm">{{ t('whitelist.cancel') }}</button>
          <button type="submit" :disabled="submitting">
            {{ submitting ? t('whitelist.saving') : t('whitelist.save') }}
          </button>
        </div>
      </form>
    </div>
