...
import com.freepark.local.softwareplate.SoftwarePlateModels.BBox;
...
public class HyperLpr3Client implements SoftwarePlateClient {
    ...
    public RecognitionResult recognize(HyperLpr3Settings settings, byte[] image, String originalName) {
    ...
    public RecognitionResult recognize(HyperLpr3Settings settings, byte[] image, String originalName,
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SOFTWARE_PLATE_UPSTREAM_FAILED, baseUrl + ": " + e.getMessage());
        }
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            ...
            throw new BusinessException(ErrorCode.SOFTWARE_PLATE_UPSTREAM_FAILED, "HTTP " + response.statusCode());
        }
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SOFTWARE_PLATE_UPSTREAM_FAILED, "bad JSON");
        }
        ...
        if (code != SUCCESS_CODE) {
            ...
            throw new BusinessException(ErrorCode.SOFTWARE_PLATE_UPSTREAM_FAILED, msg);
        }
        ...
        List<DetectedPlate> filtered = raw.stream()
        ...
        for (int i = 0; i < filtered.size(); i++) {
            DetectedPlate p = filtered.get(i);
            boolean valid = PlateShape.isValid(p.plate());
        ...
        for (int i = 0; i < scored.size(); i++) {
            ...
            DetectedPlate a = scored.get(i).plate();
            for (int j = i + 1; j < scored.size(); j++) {
                if (suppressed.get(j)) continue;
                DetectedPlate b = scored.get(j).plate();
        ...
        return new RecognitionResult(imageId, elapsedMs, resultPlates.size(), resultPlates, best,
                "cpu", baseUrl);
    }
    ...
    private record Scored(int i, DetectedPlate plate, double score, boolean valid) {}

    private DetectedPlate parsePlate(JsonNode node) {
