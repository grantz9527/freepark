...
class ParkingLotControllerTest {
    ...
    @Autowired
    private CloudUserRepository users;
  ...
  private String operatorToken() throws Exception {
        ...
        users.save(new CloudUser(
                opUsername,
                passwordEncoder.encode("secret12"),
                "Lot Operator",
                UserRole.OPERATOR));
        ...
        MvcResult operatorLogin = mockMvc.perform(post("/api/v1/auth/login")
                ...
                .andExpect(status().isOk())
                .andReturn();
        return jsonMapper.readTree(operatorLogin.getResponse().getContentAsString())
                .get("data").get("token").asString();
    }
    ...
    void operatorCannotCreateLot() throws Exception {
        String operatorToken = operatorToken();
    ...
    @Test
    void adminCanUpdateLotBilling() throws Exception {
        String token = adminToken();
        String code = "lot_" + System.nanoTime();
        ...
        MvcResult create = mockMvc.perform(post("/api/v1/lots")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"Billing Lot\",\"code\":\"" + code
                                + "\",\"lotType\":\"PUBLIC\",\"totalSpaces\":20}"))
                .andExpect(status().isOk())
                .andReturn();
        String lotId = jsonMapper.readTree(create.getResponse().getContentAsString())
                .get("data").get("id").asString();

        MvcResult plans = mockMvc.perform(get("/api/v1/billing-plans").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andReturn();
        String planId = null;
        for (var node : jsonMapper.readTree(plans.getResponse().getContentAsString()).get("data")) {
            if ("by_plate_color".equals(node.get("code").asString())) {
                planId = node.get("id").asString();
                break;
            }
        }

        mockMvc.perform(get("/api/v1/lots/" + lotId + "/billing").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.billingPlanId").value(nullValue()))
                .andExpect(jsonPath("$.data.billingPlanName").value(nullValue()));

        mockMvc.perform(put("/api/v1/lots/" + lotId + "/billing")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"billingPlanId\":\"" + planId + "\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.billingPlanId").value(planId))
                .andExpect(jsonPath("$.data.billingPlanCode").value("by_plate_color"))
