...
function openBillingConfig(lot: LotView): void {
  void router.push({ name: 'lotBilling', params: { lotId: lot.id } })
}
          ...
          <tr v-for="item in filteredLots" :key="item.id">
            ...
            <td v-if="isAdmin" class="col-actions">
              <div class="action-group">
                ...
                <button type="button" class="link-btn" @click="openBillingConfig(item)">
                  {{ t('lots.billingConfig') }}
                </button>
              </div>
            </td>
          </tr>
